...
- (void)viewDidLoad {
    ...
    self.title = Util_RotationVc_Title;
...
-(void)rotate:(UIButton*)sender
{
    [UIView animateWithDuration:0.5 animations:^{
        self.rotationBn.transform = CGAffineTransformRotate(self.rotationBn.transform, M_PI_2);
...
-(void)gestureEvent:(UITapGestureRecognizer*)gesture
{
    ...
    {
        [UIView animateWithDuration:0.5 animations:^{
            self.imgExchg.transform = CGAffineTransformMakeRotation(M_PI_2);
        } completion:^(BOOL finished) {
        }];
    }
}
